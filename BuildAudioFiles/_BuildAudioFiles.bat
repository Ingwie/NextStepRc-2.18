...
for /r %%a in ("*.wav") do CALL SOUNDS\tools\sox --norm=-3 %%a %%~na.aif silence 1 0.1 0.1% reverse
REM Cut silence
for /r %%a in ("*.aif") do CALL SOUNDS\tools\sox %%a %%~na.wav silence 1 0.1 0.1% reverse 
del *.aif /s
REM Convert to mp3
for %%a in ("*.wav") do CALL SOUNDS\tools\lame -q0 --highpass 0.08 --abr 256 %%a %%~na.mp3
md VOICEMP3
move /y *.mp3 VOICEMP3
rem del *.mp3 /s
